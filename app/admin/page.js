...
export default function Admin() {
  ...
  const [activeTab, setActiveTab] = useState("services"); // 'services' | 'queries' | 'offices'
  ...
  const handleOfficeSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      if (editingOfficeId) {
        await updateOffice(editingOfficeId, officeForm, token);
      } else {
        await createOffice(officeForm, token);
      }
      loadOffices();
      ...
      alert("Failed to save office. Please check form data.");
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-12">
          <div>
            ...
            <p className="text-xl text-gray-600 mt-2">
              Manage government services & view AI queries
            </p>
          </div>
        </div>
        ...
        <div className="flex bg-white rounded-2xl shadow-lg p-1 mb-8">
          ...
          <button
            className={`px-8 py-4 font-bold rounded-xl transition-all ${activeTab === "offices" ? "bg-gradient-to-r from-indigo-600 to-purple-600 text-white shadow-xl" : "text-gray-600 hover:text-gray-900"}`}
        ...
        {activeTab === "offices" && (
          <>
            <div className="text-center py-12 mb-8">
              <button
                onClick={loadOffices}
                className="px-8 py-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-2xl font-bold shadow-xl hover:shadow-2xl"
                disabled={loadingOffices}
              >
                {loadingOffices ? "Loading..." : "Refresh Offices"}
              </button>
            </div>
            <OfficeForm
              form={officeForm}
